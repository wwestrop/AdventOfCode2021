...
namespace AventOfCode2021.Tests
{
    public class Day10
    {
        ...
        [InlineData("]]}}]}]}>", 995444)]
        [InlineData("])}>", 294)]
        public void TestCompletionScores(string completion, ulong expectedScore)
        {
...
[([<<({({[<{(((){}){<>{}})(({}<>))}{<[[]()][{}{}]>{([][])<[][]>}}>]<<(<{<>()}[()[]]>[<(){}>[<>()>])<<[{}(";
        ...
        private ulong GetCompletionScore(string completionString)
            => completionString
                   .ToCharArray()
                   .Aggregate((ulong)0, (acc, c) => acc * 5 + GetCompletionCharacterScore(c));

        private ulong GetCompletionCharacterScore(char closingParen)
            => closingParen switch
            {
                ')' => 1,
                ']' => 2,
