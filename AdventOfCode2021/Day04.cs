...
22 11 13  6  5
 2  0 12  3  7";
            ...
            input = @"99,56,7,15,81,26,75,40,87,59,62,24,58,34,78,86,44,65,18,94,20,17,98,29,57,92,14,32,46,79,85,84,35,68,55,22,41,61,90,11,69,96,23,47,43,80,72,50,97,33,53,25,28,51,49,64,12,63,21,48,27,19,67,88,66,45,3,71,16,70,76,13,60,77,73,1,8,10,52,38,36,74,83,2,37,6,31,91,89,54,42,30,5,82,9,95,93,4,0,39
...
91 61 73 70 47";
        ...
        internal static (int[] nums, Board[] boards) ParseInput(string input)
        {
            var atoms = input.Split("\r\n\r\n");

            var nums = atoms[0].Split(",").Select(int.Parse).ToArray();
            var boards = atoms.Skip(1).Select(b => new Board(b)).ToArray();
        ...
        internal class Board
        {
            ...
            public Board(string input)
            {
                ...
                {
                    var row = rows[i];
                    var cols = row.Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
            ...
            public bool HasWon
            {
                get
                {
                    bool allMarkedSoFar = true;
                    for (int i = 0; i < SIZE; i++)
                    {
                        allMarkedSoFar = true; // reset as this row didn't have it
                        ...
                        if (allMarkedSoFar) return true;
                    }

                    for (int i = 0; i < SIZE; i++)
                    {
                        allMarkedSoFar = true; // reset as this col didn't have it
